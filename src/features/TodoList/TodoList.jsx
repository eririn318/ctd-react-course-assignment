...
function TodoList({
  ...
  const filteredTodoList = useMemo(() => {
    ...
    let filteredTodos;
    switch (
      statusFilter //1. "Look at the value inside this box..."
    ) {
    ...
    const sortedTodos = [...searchedTodos].sort((a, b) => {
      ...
      if (sortBy === "title") {
        ...
        compareResult = firstDate - secondDate;
      }
    ...
    return {
      version: dataVersion,
      todos: sortedTodos, //whatever filtered from switch case above, then sorted
    };
  }, [todoList, dataVersion, statusFilter, sortBy, sortDirection, searchTerm]); //when these things change, recalculate filteredTodoList/todoList->new todo, dataversion->dataVersion changes from 0 to 1, setFilter->"all", "active", or "completed"
  ...
  const getEmptyMessage = () => {
    //use if filteredTodoList.todos.length === 0 //ex:You clicked the "Completed" tab, but you haven't checked off any checkboxes yet. Your filtered box is totally empty.
    switch (statusFilter) {
      case "completed":
    ...
    <>
     ...
     ? : → shows message OR ul, never both, never empty ul ✅ */}
      {filteredTodoList.todos.length === 0 ? (
        <p className="text-sm text-red-500">{getEmptyMessage()}</p>
      ) : (
        <ul>
          {filteredTodoList.todos.map(
            (
              todo, //map around not checked box list
            ) => (
              <TodoListItem
                key={todo.id}
                todo={todo}
